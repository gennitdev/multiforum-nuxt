...
const { result: channelResult, loading: channelLoading, error: channelError } = useQuery(GET_CHANNEL, {
...
const shouldShowDownloads = computed(() => {
  return !bothLoading.value && !anyError.value && serverDownloadsEnabled.value && channelDownloadsEnabled.value;
});
  ...
  <div>
    ...
    <div v-else-if="!shouldShowDownloads" class="px-4 py-8">
      <div class="max-w-md mx-auto text-center">
        ...
        <div v-if="!channelDownloadsEnabled && serverDownloadsEnabled" class="text-sm text-gray-500 dark:text-gray-400">
          Contact the forum administrators to enable downloads for this forum.
        </div>
        <div v-else-if="!serverDownloadsEnabled" class="text-sm text-gray-500 dark:text-gray-400">
