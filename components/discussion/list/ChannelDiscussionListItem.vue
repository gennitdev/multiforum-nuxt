...
const detailLink = computed(() => {
  ...
  return `/forums/${defaultUniqueName.value}/discussions/${props.discussionChannel.discussionId}`;
});
...
<template>
  <li class="relative my-1 flex md:rounded-lg border dark:border-gray-700 bg-white p-2 dark:bg-gray-800">
    <div class="flex w-full flex-row justify-start gap-4 rounded-lg p-1">
      <div v-if="discussion" class="w-full flex-col">
        <div class="flex">
          <div class="mr-2 flex h-10 w-10 justify-center items-center rounded-md bg-gray-100 p-1 text-xl dark:bg-gray-600">
            💬
          </div>
          <div class="w-full flex-col">
            <router-link :to="{ path: detailLink, query: filteredQuery }" class="w-full">
              <span class="cursor-pointer hover:text-gray-500 dark:text-gray-100">
                <HighlightedSearchTerms :text="title" :search-input="searchInput" :classes="'font-medium text-md'" />
              </span>
            </router-link>
            <div class="font-medium py-1 text-xs text-gray-600 no-underline dark:text-gray-300">
              <span class="mr-1 text-xs">{{ `Posted ${relativeTimeAgo} by` }}</span>
              <UsernameWithTooltip
                v-if="authorUsername"
        ...
        <div v-if="discussion?.body" class="my-2 border-gray-400 dark:bg-gray-700">
          <MarkdownPreview :text="discussion.body" :disable-gallery="false" :word-limit="50" class="-ml-2" />
        </div>
        <div class="font-medium my-1 flex space-x-1 text-xs text-gray-600 hover:no-underline">
          <TagComponent v-for="tag in tags" :key="tag" class="my-1" :active="selectedTags.includes(tag)" :tag="tag" @click="$emit('filterByTag', tag)" />
        </div>
        <div class="flex items-center">
          <DiscussionVotes v-if="discussionChannel" :discussion="discussion" :discussion-channel="discussionChannel" :show-downvote="false" />
          <div class="flex items-center justify-start gap-6">
            <router-link :to="{ path: detailLink, query: filteredQuery }" class="rounded-md p-1 px-4 underline">
              <i class="fa-regular fa-comment h-5 w-5" />
              <span class="text-md">{{ `${commentCount} comment${commentCount === 1 ? "" : "s"}` }}</span>
            </router-link>
          </div>
        </div>
      </div>
